...
Trap.ClientEndpoint = function(configuration, autoConfigure)
{
	...
	{
		this.useBinary = typeof(configuration.useBinary) == "boolean" ? configuration.useBinary : false;
		
		this.autoConfigure = (typeof(configuration.autoConfigure) == "boolean" ? configuration.autoConfigure : true);
	...
	else
	{
		...
		this.useBinary = typeof(Trap.useBinary) != "undefined" ? Trap.useBinary : Trap.supportsBinary;
	}
	...
	this.trapID = Trap.Constants.ENDPOINT_ID_CLIENT; // Allow the server to override our trap ID.
	...
	if ((configuration != null) && (configuration.trim().length > 0))
	{
		...
		else if (configuration.startsWith("http://") || configuration.startsWith("https://"))
		{
...
Trap.ClientEndpoint.prototype.sendOpen = function(transport)
{
	...
	body.setOption(Trap.Constants.OPTION_ENABLE_COMPRESSION, "true"); // Forcibly enable compression for the time being
	
	if (!!this.config.getOption(Trap.Constants.OPTION_AUTO_HOSTNAME))
		body.setOption(Trap.Constants.OPTION_AUTO_HOSTNAME, this.config.getOption(Trap.Constants.OPTION_AUTO_HOSTNAME)); 
				...
				if (mt.transportsToConnect.size() > 0)
				{
					try
					{
						...
						if (first != null)
						{
							...
							for (var i=0; i<mt.availableTransports.size(); i++)
								if (mt.availableTransports.get(i).getTransportPriority() < t.getTransportPriority())
							...
							t.onmessagesent = function(e){ mt.ttMessageSent(e.message, e.target, null); };
